...
## This script will run a pseudobulk eQTL analysis on multiple datasets at once, conditioning on the lead variant.
## Input: PEER-normalized residuals for each dataset
## Output: HLA conditional (secondary) eQTLs
...
for (gene in HLA_genes) {
    ...
    # Get lead variant for the gene (to condition on)
    lead_variant = lead_variants[which(lead_variants$gene == str_replace(gene, '\\.', '-')), 'variant'] %>% unlist()
    message('Conditioning on lead variant: ', lead_variant)
    
    variants_to_test = colnames(data)[10:ncol(data)] # variants start in 10th row of data
    variants_to_test =  variants_to_test[variants_to_test != lead_variant] # remove lead
    
    results = rbindlist(lapply(variants_to_test, function(variant) { 
        mod = lm(as.formula(paste(gene, ' ~ dataset + ', lead_variant, '+',
        paste(variant, collapse = "+"), sep = "")), data = data)
        
        # if variant is in perfect LD with lead, then may not be possible to estimate beta
...
write.csv(full_results, paste0(outprefix, '_secondary_all_variants.csv'))
write.csv(lead_variants, paste0(outprefix, '_secondary_lead_variants.csv'))