...
## This script will run a pseudobulk eQTL analysis on multiple datasets at once, conditioning on the lead and secondary variant.
...
for (gene in HLA_genes) {
    ...
    message('Conditioning on lead variant: ', lead_variant, secondary_lead_variant)
    ...
    variants_to_test =  variants_to_test[variants_to_test != lead_variant &
                                         variants_to_test != secondary_lead_variant] # remove leads
    
    results = rbindlist(lapply(variants_to_test, function(variant) { 
        mod = lm(as.formula(paste(gene, ' ~ dataset + ', lead_variant, '+', secondary_lead_variant, '+',
...
write.csv(full_results, paste0(outprefix, '_tertiary_all_variants.csv'))
write.csv(lead_variants, paste0(outprefix, '_tertiary_lead_variants.csv'))