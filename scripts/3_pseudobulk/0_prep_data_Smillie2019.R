...
# Perform cell QC for Smillie dataset
...
cell_meta = read.csv(paste0(prefix, 'meta/cell_meta_Smillie_completeHLA.csv'), row.names = 1)
genesXcells_pers = readRDS(paste0(prefix, 'genesXcells_GeneFull_Ex50pAS/exp_Smillie_pers_EM_GeneFull_Exon50pAS.rds'))
...
write.csv(cell_meta, paste0(prefix, 'meta/cell_meta_Smillie_completeHLA_cellQC.csv'), quote = F)

##### Write matrices

# Read in data
sample_meta = read.csv(paste0(prefix, 'meta/sample_meta_Smillie_completeHLA.csv'), row.names = 1)
cell_meta = read.csv(paste0(prefix, 'meta/cell_meta_Smillie_completeHLA_cellQC.csv'), row.names = 1)
genesXcells_pers = readRDS(paste0(prefix, 'genesXcells_GeneFull_Ex50pAS/exp_Smillie_pers_EM_GeneFull_Exon50pAS.rds'))
genesXcells_noPers = readRDS(paste0(prefix, 'genesXcells_GeneFull_Ex50pAS/exp_Smillie_noPers_noMM_GeneFull_Exon50pAS.rds'))
...
for (c in unique(cell_meta$cell_type_major)) {
    ...
    saveRDS(cell_meta_c, paste0(prefix_out, '0_data/cell_meta_Smillie_', c, '.rds'))
    saveRDS(genesXcells_pers_c[, idx_keep], paste0(prefix_out, '0_data/exp_pers_Smillie_', c, '.rds'))
    saveRDS(genesXcells_noPers_c[, idx_keep], paste0(prefix_out, '0_data/exp_noPers_Smillie_', c, '.rds'))
}

message('Done with Smillie2019')