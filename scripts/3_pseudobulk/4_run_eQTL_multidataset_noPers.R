...
## Runs pseudobulk eQTL analysis on multiple datasets at once using standard pipeline expression estimates.
...
message('read noPers inputs')
AMP2RA_resid_path = paste0(prefix, 'eqtl_pseudobulk/2_PEER/noPers_AMP2RA_', cell_type, '_K7_PEER_residuals.csv')
OneK1K_resid_path = paste0(prefix, 'eqtl_pseudobulk/2_PEER/noPers_OneK1K_', cell_type, '_K20_PEER_residuals.csv')
Smillie_resid_path = paste0(prefix, 'eqtl_pseudobulk/2_PEER/noPers_Smillie_', cell_type, '_K2_PEER_residuals.csv')
Randolph_NI_resid_path = paste0(prefix, 'eqtl_pseudobulk/2_PEER/noPers_Randolph_NI_', cell_type, '_K7_PEER_residuals.csv')
...
write.csv(full_results, paste0(outprefix, '_pseudobulk_eQTLs_noPers.csv'), quote = F)
saveRDS(resid, paste0(outprefix, '_residuals_noPers.rds'))
saveRDS(geno_dosage, paste0(outprefix, '_sampleXdosage_noPers.rds'))