...
# Given a cell type, raw counts, a single variant, and gene, this script will test whether the regulatory variant is an eQTL using the NBME model for the AMP2RA dataset.
...
null_model <- lme4::glmer.nb(formula = E ~ (1 | IND) + # no genotype term
                             AGE + SEX + nUMI + MT + 
                             gPC1 + gPC2 + gPC3 + gPC4 + gPC5 + 
                             expPC1 + expPC2 + expPC3 + expPC4 + expPC5, 
                             data = data, nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))
...
outprefix = '/data/srlab1/jkang/hla2023/eqtl_single_cell/1_sceQTL_nostate/AMP2RA_NBME_'
