...
# Tests for calibration of cell state interaction analysis using Poisson ME model, for a given cell type, gene, and variant, using the OneK1K dataset and OneK1K-defined hPC embedding.
...
for (i in 1:nperms) {
    ...
    message('Fit Full PME model')
    full_model <- lme4::glmer(formula = E ~ G + (1 | IND) + (1 | B) +
                        AGE + SEX + nUMI + MT + 
    ...
    message('Fit Null PME model')
    null_model <- lme4::glmer(formula = E ~ G + (1 | IND) + (1 | B) +
                        AGE + SEX + nUMI + MT + 
                        gPC1 + gPC2 + gPC3 + gPC4 + gPC5 + 
                        expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + 
...
saveRDS(results, paste0(outprefix, paste('PME_OneK1K_hPCS_', cell_type, gene, snp, 
                                         nperms, 'perms_acrossDonors', sep = '_'), '.rds'))
saveRDS(waldPs, paste0(outprefix, paste('PME_OneK1K_hPCS_', cell_type, gene, snp, 
